...
public class AsoController {
	...
	@PostMapping("/aso/filtro")
	public ResponseEntity<?> asoFiltro(@RequestBody AsoFiltroDTO asoDto){
		
		List<Aso> listaAsoRetorno = as.filtroAso(asoDto);
		
		if(listaAsoRetorno == null) {
			return new ResponseEntity<Aso>(HttpStatus.valueOf(400));
		}
		
		return new ResponseEntity<List<Aso>>(listaAsoRetorno, HttpStatus.OK);
	}
